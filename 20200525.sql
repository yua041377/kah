...
PL/SQL : 호불호가 많이 갈림, 분명히 필요한 경우는 있음(로직이 복잡하고 대다수의 부분이 데이터와 관련되었을 때)
...
4] cycle 테이블(애음요일)을 이용하여 daily 테이블에 데이터를 생성 해주는 프로시져
여사님 한분이 관리하는 고객 2~300명
고객한명당 제품을 2~3개 정도 애음
일실적 (한달 20일이 영업일) 40000~60000건 정도의 데이터

cycle 테이블
cid, pid,    day,    cnt
1    100  2020/05/04  1 : 1번 고객이 100번 제품을 2020/05/04 월요일날 1개 먹는다
1    100  2020/05/11  1 : 1번 고객이 100번 제품을 2020/05/11 월요일날 1개 먹는다
1    100  2020/05/18  1 : 1번 고객이 100번 제품을 2020/05/18 월요일날 1개 먹는다
1    100  2020/05/25  1 : 1번 고객이 100번 제품을 2020/05/25 월요일날 1개 먹는다
...
--    커서를 루프를 돌면서(outer loop)
--    해당 고객의 애음요일정보와 일치하는 달력테이블의 일자를 찾아서
...
mybatic와 같은 ORM(Object Realtion mapping) 프레임워크 에서도 jdbc 프로그래밍에서 반드시 들어가야 하는
SQLException(checked exception)을 개발 편의를 위해 프레임워크에서 정의한 예외로 치환하여 사용한다.

SELECT 결과가 없을 때 ==> NO_DATA_FOUND

아래 쿼리는 특정 직원을 조회하기 위핸 쿼리
...
사원번호를 입력받아 해당사원의 이름을 리턴하는 function : getEmpName 생성
