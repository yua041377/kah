...
reg_dt 컬럼은 값을 별도 입력하지 않았지만 테이블 설정에 의해
기본값 SYSDATE가 입력이 된다;

member 테이블 성별이라는 컬럼이 있을 때
성별 칼럼에 들어갈 수 있는 값 : 남성, 여성, 남여
...
FOREIGN KEY : 해당 컬럼이 참조하는 테이블의 값이 존재하는지 확인하는 제약
            EX : emp 테이블에 신규사원을 등록시 deptno 같은 dept 테이블에 반드시 존재하는 값만 등록이 가능하다.
CHECK : 컬럼에 입력되는 값을 개발자가 정 한 로직에 의해 체크, 검증 하는 제약
EX : 성별 컬럼에 같은 F, M 두개의 값만 들어갈 수 있도록 설정
...
1. 테이블 생성 시 컬럼 옆에 제약조건을 기술
DESC dept;

부서번호는 중복이 되면 안되고, 값이 비어 있어서도 안된다. 일반적으로는
==> dbms에서 설정 가능한 PRIMARY KEY 제약조건을 컬럼 레벨로 설정

제약조건 이름을 기술하지 않을 경우 오라클 DBMS가 자동으로 제약조건 이름을 붙여 준다.
...
위 쿼리는 첫번째 쿼리에서 입력한 부서번호와 중복되기 때문에 PRIMARY KEY(UNIQUE) 제약조건에
위배되어 정상적으로 데이터가 입력되지 않는다.
