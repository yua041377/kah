...
만약 동일한 SQL이 실행될 경우 작성된 실행계획이 존재할 경우
...
Multiple insert : 여러건의 데이터를 동시에 입력하는 insert의 확장 구문
...
emp테이블에는 존재하고 emp_test 테이블에는 존재하지 않는 직원을 신규 입력
emp테이블에는 존재하고 emp_test 테이블에도 존재하지 않는 직원을 변경
...
(사용자가 9999번, james 사원을 등록하거나, 업데이트 하고 싶을 때)
...
GROUP BY를 사용하는 두개의 SQL로 나눠서 하나로 합치는 (UNION) 작업을 진행
