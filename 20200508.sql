...
4. CHECK
    . NOT NULL
5. NOT NULL
    CHECK 제약 조건이지만 많이 사용하기 때문에 별도의 KEYWORD 를 제공
...
FOREIGN KEY 제약조건은 참조하는 테이블이 있기 때문에 두개의 테이블 간 설정한다.
...
98번 부서는 존재하지 않으므로 정상적으로 입력 할 수 없다.
...
emp_test.deptno ==> dept.deptno 참조 중.
...
부모 데이터를 삭제 시..
FOREIGN KEY 옵션에 따라 지식 데이터를 처리할 수 있는 옵션
...
java코드에는 dept테이블만 삭제하는 코드가 있는데 신기하게도 emp 테이블의 데이터가 삭제되거나 null로 설정될 경우를 볼 수 있음.

CHECK 제약조건
컬럼의 값을 제한하는 제약 조건

emp 테이블에서 급여정보(sal)을 관리 할 때 sal 컬럼의 값은 0보다 작은 값이 들어가는 것은
로직적으로 이상함.
...
컬럼 이름 변경
ALTER TABLE emp_test rename COLUMN 기존컬럼명 to 신규컬럼명;

위에서 추가한 emp_test 테이블의 hp 컬럼을 hp_n 으로 이름 변경
...
DDL : CREATE, ALTER == 트랜잭션 제어 불가능 (취소가 안된다.)
